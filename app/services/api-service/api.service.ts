...
public disconnect(): void {
  ...
  onError?:(data:string)=>void // This is the error callback passed by the consumer/test
){
  this.stateMaybe()
  ...
  cmdConnect.onError = (error) => {
    this.stateBad();
    if(onError) {onError(error)} // This ensures the consumer's onError is called
  };
 
  cmdConnect.onReciveConnectBegin = (message) => {
    if (message.status.Err){
      this.stateBad()
      // Now cmdConnect.onError *is* defined, and it correctly points to the logic
      // that calls the 'onError' passed into the Connect method.
  ...
  // **** REMOVE THIS DUPLICATE ASSIGNMENT ****
  // cmdConnect.onError = (error) => {
  //   this.stateBad();
  //   if(onError) {onError(error)}
  // }
