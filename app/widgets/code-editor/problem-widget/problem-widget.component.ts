...
import { ServiceDescriptor, ProblemDescriptor, ArgsMap, FilesMap, FileDescriptor, ArgDescriptor } from '../../../services/problem-manager-service/problem-manager.types';
...
private subscriptions: Subscription[] = []; // Add this property
              ...
              )
  {
    ...
    this.subscriptions.push(this.pm.onProblemsChanged.subscribe((clear:boolean)=>{ this.problemsDidChange(clear) }));
    this.subscriptions.push(this.api.onApiStateChange.subscribe((state:ApiState)=>{this.updateState(state)}));
    this.subscriptions.push(this.pm.onError.subscribe((_)=>{this.stateBad()}));
    this.url = ""
    this.lastUrl = ""
...
async validateArgs(): Promise<Map<string, any> | undefined> {
  let result: Map<string, any> | undefined = undefined;
  if (this.selectedService) {
    result = this.pm.validateArgs(this.selectedService);
  }
  return result;
}
...
public async didSelectProblem(p0: any) {
    this.selectedService = undefined;
    this.selectedArgs = undefined;
    this.selectedFiles = undefined;
...
selectProblem(problem: ProblemDescriptor): void {
  this.selectedProblem = problem;
  this.onProblemSelected.emit(problem);
}
