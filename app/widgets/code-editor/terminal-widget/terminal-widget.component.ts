...
export class TerminalWidgetComponent implements OnInit {
  ...
  public response!: string; // Using '!' to assert it will be initialized

  public onError = new EventEmitter<any>();
  public onProblemsChanged = new EventEmitter();
  public onResponseComplete = new EventEmitter(); // This is the EventEmitter that signals response is ready
...
ngOnInit() {
  ...
  onGetCommand() {
    ...
    this.response = ''; // Clear response before API call, it will be set in onData or onError
    this.selectedProblem = new ProblemDescriptor(this.problemSearch, new Meta(''));
    this.onProblemSelected.emit(this.selectedProblem);
    // DO NOT emit onResponseComplete here, it should be emitted by the API callbacks (onData or onError)
    // this.onResponseComplete.emit();
  }
  ...
  onConnectCommand() {
    switch (this.api.setUrl(this.url)) {
      case 0:
        ...
        this.prompt = "";
        this.runConnectAPI(); // This is an async call, response should be handled by its callbacks
        break;
    ...
    // DO NOT emit here if runConnectAPI is handling response
    // this.onResponseComplete.emit();
  }
    ...
    switch (this.api.setUrl(this.url)) {
      case 0:
        ...
        let onData = (data: ArrayBuffer) => {
          ...
          this.response = "Attachment downloaded successfully!"; // Set response on success
          this.onResponseComplete.emit(); // Emit on success
        };

        let req = this.api.GetAttachment(this.problemSearch, onAttachment, onAttachmentInfo, onData);
        req.onError = (error: string) => { // Added type for error
          this.response = "ERROR Cannot download attachment: Problem '" + this.problemSearch + "' does not exists: " + error; // Include error message
          this.onResponseComplete.emit(); // Emit on error
        };
        break;
    ...
    switch (this.api.setUrl(url)) {
      case 0:
        let req = this.api.problemList((problemMap) => {
          ...
          this.onListCommand(); // onListCommand will set response and emit
        });

        req.onError = (error: string) => { // Added type for error
          this.response = "ERROR: Cannot connect to '" + this.url + "': " + error; // Include error message
          this.onResponseComplete.emit(); // Emit on error
        };
        break;
      case -1:
  ...
  onListCommand() {
    ...
    this.onResponseComplete.emit(); // Ensure it's emitted once after full response is built
  }
